...
- Vendored the two `fix dplr` patches into `dp-patch/`, replacing the `wget` of a
  ...
  - `0001-make-fele-public-and-can-be-modified-by-external-mod.patch` — makes
    `FixDPLR::dfele` public. Required to build `src/lmp`, which writes to it from
    `verlet_split_dplr.cpp`.
  - `0001-fix-lmp-place-Wannier-centroids-in-setup_post_neighb.patch` — places the
    Wannier centroids in `setup_post_neighbor()` instead of `setup_pre_force()`, so
    a fix defined after `fix dplr` (such as `fix electrode/conp`) sees the real
    centroid positions while it sets itself up. Also adds `MIN_POST_NEIGHBOR` to the
    fix mask, which `minimize` needs.
